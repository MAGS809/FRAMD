...
class SkeletonSegment:
    """A single segment in the motion/timing skeleton extracted from reference."""
    segment_id: int
    start_time: float       # seconds
    end_time: float         # seconds
    duration: float         # seconds
    motion_type: str        # static, pan, zoom, tracking, handheld, etc.
    camera_movement: str    # push_in, pull_out, pan_left, pan_right, tilt_up, static, etc.
    subject_movement: str   # walking, talking, gesture, still, action, etc.
    cut_style: str          # hard_cut, dissolve, whip, jump_cut
...
class SkeletonStructure:
    """
    Motion/timing skeleton extracted from reference video.
    This is the STRUCTURE ONLY - no visuals from reference enter the final video.
    """
    total_duration: float
    segments: List[SkeletonSegment]
    beat_markers: List[float]       # timing beats for sync
    transition_points: List[float]  # where cuts happen
    pacing_curve: str               # how energy flows: build, plateau, wave, decline
    
    
class VisualSource(Enum):
    """Source type for each scene's visuals."""
    STOCK = "stock"           # From Pexels/stock library
    CONTENT = "content"       # User's uploaded content files
    RUNWAY = "runway"         # AI-generated by Runway
    GENERATED = "generated"   # DALL-E or other AI images


@dataclass
class SceneSourceAssignment:
    """
    Explicit assignment of visual source for each scene.
    Claude decides this - Runway/Shotstack just execute.
    """
    scene_id: int
    source_type: VisualSource
    source_reason: str          # why this source was chosen
    stock_query: Optional[str]  # if stock, what to search for
    content_file_id: Optional[str]  # if content, which file
    runway_prompt: Optional[str]    # if runway, the generation prompt
    base_image_url: Optional[str]   # image for runway to transform
    duration: float
    motion_guidance: str        # how motion should flow (from skeleton)
...
class OrchestrationPlan:
    ...
    skeleton: Optional[SkeletonStructure]
    source_assignments: List[SceneSourceAssignment]
...
def extract_skeleton_from_reference(
    reference_file: Optional[Dict[str, Any]] = None,
    user_description: Optional[str] = None,
    target_duration: float = 30.0
) -> SkeletonStructure:
    """
    Extract motion/timing SKELETON from reference video.
    
    CRITICAL: This extracts STRUCTURE ONLY - no visuals from reference enter final video.
    The skeleton defines HOW the video moves, not WHAT it shows.
    
    Args:
        reference_file: Dict with video reference info
        user_description: Optional description of desired structure
        target_duration: Target duration for output video
        
    Returns:
        SkeletonStructure with timing and motion map
    """
    from context_engine import call_ai, SYSTEM_GUARDRAILS
    ...
    if reference_file:
        file_context = f"""
REFERENCE VIDEO (for SKELETON extraction only - visuals will NOT be used):
- Name: {reference_file.get('name', 'unknown')}
- Type: {reference_file.get('type', 'video')}

This video's VISUALS will NEVER appear in the final output.
We are extracting only:
1. When cuts happen (timing structure)
2. How the camera moves (motion patterns)
3. How subjects move (movement types)
4. The rhythm and pacing (energy flow)
"""
    
    prompt = f"""Analyze this reference to extract its MOTION/TIMING SKELETON.

CRITICAL RULE: We are extracting STRUCTURE, not visuals. The reference video's 
actual footage will NEVER be used. We only want to understand:
- WHEN things happen (timing)
- HOW camera moves (motion type)
- WHERE cuts land (transitions)
{file_context}

User notes: {user_description or "No specific notes"}
Target duration: {target_duration} seconds

Break the structure into segments. For each segment identify:
1. segment_id: Sequential number
2. start_time: When segment begins (seconds)
3. end_time: When segment ends (seconds)  
4. duration: Length in seconds
5. motion_type: static, pan, zoom, tracking, handheld, dolly, crane, aerial
6. camera_movement: push_in, pull_out, pan_left, pan_right, tilt_up, tilt_down, static, orbit
7. subject_movement: still, walking, talking, gesture, action, dance, reveal
8. cut_style: hard_cut, dissolve, whip, jump_cut, fade
9. intensity: 0.0 (calm) to 1.0 (intense)

Also identify:
- beat_markers: Key timing points for audio sync (array of seconds)
- transition_points: Where cuts happen (array of seconds)
- pacing_curve: Overall energy flow (build, plateau, wave, decline)
...
{{
    ...
    "segments": [
        {{
            ...
            "subject_movement": "still",
            "cut_style": "fade",
            "intensity": 0.3
        }}
    ],
    "beat_markers": [0.0, 5.0, 10.0, 15.0],
    "transition_points": [5.0, 10.0, 15.0, 20.0],
    "pacing_curve": "build"
}}"""
    ...
    if result and result.get("segments"):
        for seg in result["segments"]:
            segments.append(SkeletonSegment(
                ...
                motion_type=seg.get("motion_type", "static"),
                camera_movement=seg.get("camera_movement", "static"),
                subject_movement=seg.get("subject_movement", "still"),
                cut_style=seg.get("cut_style", "hard_cut"),
                intensity=float(seg.get("intensity", 0.5))
            ))
    
    if not segments:
        segments = [SkeletonSegment(
            segment_id=1, start_time=0, end_time=target_duration,
            duration=target_duration, motion_type="static",
            camera_movement="static", subject_movement="still",
            cut_style="fade", intensity=0.5
        )]
    
    return SkeletonStructure(
        total_duration=result.get("total_duration", target_duration) if result else target_duration,
        segments=segments,
        beat_markers=result.get("beat_markers", []) if result else [],
        transition_points=result.get("transition_points", []) if result else [],
        pacing_curve=result.get("pacing_curve", "constant") if result else "constant"
    )
...
) -> List[SceneSourceAssignment]:
    """
    SURGICAL SOURCE ASSIGNMENT: Claude decides exactly which visual source to use for each scene.
    
    Priority order:
    1. Content files (user's actual footage) - HIGHEST priority, use when available
    2. Runway (AI-generated) - For abstract, stylized, or unavailable scenes
    3. Stock (Pexels) - For realistic b-roll and gap-filling
    
    CRITICAL: Reference files are NEVER assigned as sources. They provided the skeleton only.
    
    Args:
        skeleton: Motion/timing skeleton from reference
        vibe: Vibe profile for style guidance
        content_files: User's content files to prioritize
        topic: Content topic for stock searches
        script_segments: Script with content for each segment
        
    Returns:
        List of SceneSourceAssignment with explicit source per scene
    """
    from context_engine import call_ai, SYSTEM_GUARDRAILS
    
    safe_content = filter_reference_files(content_files) if content_files else []
    
    content_context = ""
    if safe_content:
        content_context = f"""
USER'S CONTENT FILES (PRIORITIZE THESE):
{json.dumps([{"id": c.get("id"), "name": c.get("name"), "type": c.get("type")} for c in safe_content], indent=2)}

These are the user's actual files. Use them wherever they fit the skeleton's motion requirements.
"""
    
    skeleton_context = []
    for seg in skeleton.segments:
        skeleton_context.append({
            "scene_id": seg.segment_id,
            "duration": seg.duration,
            "motion_type": seg.motion_type,
            "camera_movement": seg.camera_movement,
            "subject_movement": seg.subject_movement,
            "intensity": seg.intensity
        })
    
    prompt = f"""Assign visual sources for each scene in this video.

SKELETON STRUCTURE (from reference - motion/timing only, no visuals):
{json.dumps(skeleton_context, indent=2)}
...
SCRIPT SEGMENTS: {json.dumps(script_segments) if script_segments else "None specified"}

VISUAL SOURCE OPTIONS:
1. "content" - Use user's content file (HIGHEST priority if available and fits)
2. "runway" - AI-generate the visual (for abstract, stylized, or custom needs)
3. "stock" - Search Pexels for realistic b-roll

DECISION RULES:
- If user has content that matches the scene's motion/subject needs → use "content"
- If scene needs abstract, stylized, or doesn't exist in stock → use "runway"  
- If scene needs realistic footage (nature, city, people) → use "stock"
- NEVER reference the original reference video - it provided structure only

For each scene, output:
{{
    "scene_id": 1,
    "source_type": "content|runway|stock",
    "source_reason": "Why this source was chosen",
    "stock_query": "search query for Pexels (only if stock)",
    "content_file_id": "file ID (only if content)",
    "runway_prompt": "Detailed generation prompt (only if runway)",
    "base_image_url": "Image URL for runway to transform (if runway)",
    "motion_guidance": "How motion should flow to match skeleton"
}}
...
{{
    "assignments": [
        {{
            "scene_id": 1,
            "source_type": "runway",
            "source_reason": "Abstract concept needs AI generation",
            "stock_query": null,
            "content_file_id": null,
            "runway_prompt": "Cinematic slow push into abstract shapes",
            "base_image_url": null,
            "motion_guidance": "Slow push in, 5 seconds, calm intensity"
        }}
    ],
    "source_summary": {{
        "content_scenes": 0,
    ...
    if result and result.get("assignments"):
        for assign in result["assignments"]:
            source_type_str = assign.get("source_type", "stock")
            try:
                source_type = VisualSource(source_type_str)
            except ValueError:
                source_type = VisualSource.STOCK
            
            seg = next((s for s in skeleton.segments if s.segment_id == assign.get("scene_id")), None)
            duration = seg.duration if seg else 5.0
            
            assignments.append(SceneSourceAssignment(
                scene_id=assign.get("scene_id", 0),
                source_type=source_type,
                source_reason=assign.get("source_reason", ""),
                stock_query=assign.get("stock_query"),
                content_file_id=assign.get("content_file_id"),
                runway_prompt=assign.get("runway_prompt"),
                base_image_url=assign.get("base_image_url"),
                duration=duration,
                motion_guidance=assign.get("motion_guidance", "")
            ))
    
    if result:
        summary = result.get("source_summary", {})
        print(f"[Source Assignment] Content: {summary.get('content_scenes', 0)}, "
              f"Runway: {summary.get('runway_scenes', 0)}, Stock: {summary.get('stock_scenes', 0)}")
...
def build_timeline_from_assignments(
    ...
    source_assignments: List[SceneSourceAssignment],
    vibe: VibeProfile,
    fetched_assets: Dict[str, Dict[str, Any]],
    audio_track: Optional[Dict[str, Any]] = None,
    captions: Optional[List[Dict[str, Any]]] = None
) -> List[ShotStackClip]:
    """
    Build Shotstack timeline from source assignments mapped to skeleton timing.
    
    CRITICAL: This function only uses assets from source_assignments.
    Reference video visuals NEVER enter this pipeline - only skeleton timing is used.
    
    Args:
        skeleton: Motion/timing skeleton (structure only, no visuals)
        source_assignments: Claude's decisions on which source per scene
        vibe: Vibe profile for transition/effect choices
        fetched_assets: Dict mapping scene_id to fetched asset URLs
                       Keys: "runway_{scene_id}", "stock_{scene_id}", "content_{scene_id}"
        audio_track: Audio to include
        captions: Caption data
        
    Returns:
        List of ShotStackClip with precise timing from skeleton
    """
    clips = []
    ...
    base_transition = transition_map.get(vibe.cut_rhythm, ("fade", "fade"))
    
    for assign in source_assignments:
        scene_id = assign.scene_id
        
        seg = next((s for s in skeleton.segments if s.segment_id == scene_id), None)
        if not seg:
            continue
        
        asset_key = f"{assign.source_type.value}_{scene_id}"
        asset_info = fetched_assets.get(asset_key, {})
        asset_url = asset_info.get("url", "")
        
        if not asset_url:
            print(f"[Timeline] WARNING: No asset URL for scene {scene_id} ({assign.source_type.value})")
            continue
        
        transition_in, transition_out = base_transition
        if seg.cut_style == "hard_cut":
            transition_in, transition_out = "none", "none"
        elif seg.cut_style == "dissolve":
            transition_in, transition_out = "fade", "fade"
        elif seg.cut_style == "whip":
            transition_in, transition_out = "slide", "slide"
        
        effects = []
        if vibe.color_feel == "warm":
            effects.append("warmColor")
        elif vibe.color_feel == "cool":
            effects.append("coolColor")
        elif vibe.color_feel == "muted":
            effects.append("desaturate")
        
        if seg.camera_movement == "push_in":
            effects.append("zoomIn")
        elif seg.camera_movement == "pull_out":
            effects.append("zoomOut")
        ...
        print(f"[Timeline] Scene {scene_id}: {assign.source_type.value} @ {seg.start_time}s-{seg.end_time}s")
    
    if audio_track and audio_track.get("url"):
        clips.append(ShotStackClip(
            asset_url=audio_track["url"],
            asset_type="audio",
            start_time=0.0,
            duration=skeleton.total_duration,
            track=10,
            position_x=0.5,
            position_y=0.5,
            scale=1.0,
    ...
    print(f"[Timeline] Built {len(clips)} clips from {len(source_assignments)} source assignments")
    return clips
...
def create_orchestration_plan(
    ...
    SURGICAL ORCHESTRATION FLOW:
    1. Validate files - Reference blocked from integration, content prioritized
    2. Extract SKELETON from reference - motion/timing structure ONLY (no visuals)
    3. Extract VIBE from reference - mood/energy direction
    4. Assign SOURCES per scene - Claude decides stock/content/runway for each
    5. Generate Runway instructions based on source assignments
    6. Build stock queries for stock-assigned scenes
    
    The reference video's VISUALS are NEVER used - only its skeleton (structure).
    
    Args:
        user_input: User's content/idea
        reference_files: Files for skeleton/vibe extraction ONLY (blocked from integration)
        content_files: User's actual content to prioritize in final video
        script_segments: Script with timing
        target_duration: Target video duration in seconds
        
    Returns:
        Complete orchestration plan with surgical source assignments
    """
    print("[Orchestration] Starting SURGICAL plan creation...")
    print("[Orchestration] Reference = skeleton only, Content = prioritized, Stock = gap-fill")
    ...
    skeleton = extract_skeleton_from_reference(reference, user_input, target_duration)
    print(f"[Orchestration] Skeleton extracted: {len(skeleton.segments)} segments, {skeleton.pacing_curve} pacing")
    ...
    source_assignments = assign_visual_sources(
        skeleton=skeleton,
        vibe=vibe,
    ...
    print(f"[Orchestration] Source assignments complete: {len(source_assignments)} scenes")
    
    runway_instructions = []
    stock_queries = []
    
    for assign in source_assignments:
        if assign.source_type == VisualSource.RUNWAY:
            runway_instructions.append(RunwayInstruction(
                scene_id=assign.scene_id,
                generation_type="image_to_video" if assign.base_image_url else "text_to_video",
                prompt=assign.runway_prompt or "",
                duration=assign.duration,
                style_reference=f"{vibe.mood} mood, {vibe.color_feel} colors",
                motion_guidance=assign.motion_guidance,
                blend_with=[]
            ))
        elif assign.source_type == VisualSource.STOCK and assign.stock_query:
            stock_queries.append({
                "scene_id": assign.scene_id,
                "query": assign.stock_query,
                "duration": assign.duration
            })
    
    print(f"[Orchestration] Runway scenes: {len(runway_instructions)}, Stock scenes: {len(stock_queries)}")
    
    total_duration = skeleton.total_duration
...
def execute_orchestration(
    ...
    reference_files: Optional[List[Dict[str, Any]]] = None,
    wait_for_completion: bool = True,
    require_complete_assets: bool = True
) -> Dict[str, Any]:
    """
    ...
    fetched_assets: Dict[str, Dict[str, Any]] = {}
    reference_file_urls: set = set()
    reference_file_ids: set = set()
    
    if reference_files:
        for f in reference_files:
            if f.get("url"):
                reference_file_urls.add(f.get("url"))
            if f.get("id"):
                reference_file_ids.add(f.get("id"))
            if f.get("name"):
                reference_file_ids.add(f.get("name"))
    ...
    if reference_file_urls or reference_file_ids:
        print(f"[Execute] Reference file registry: {len(reference_file_urls)} URLs, {len(reference_file_ids)} IDs blocked")
    
    if plan.source_assignments:
        print("[Execute] Building fetched_assets map from source assignments...")
        
        for assign in plan.source_assignments:
            scene_id = assign.scene_id
            asset_key = f"{assign.source_type.value}_{scene_id}"
            
            if assign.source_type == VisualSource.RUNWAY:
                runway_output = next(
                    (r for r in results["runway_outputs"] if r.get("scene_id") == scene_id),
                    None
                )
                if runway_output and runway_output.get("url"):
                    fetched_assets[asset_key] = {
                        "url": runway_output["url"],
                        "duration": runway_output.get("duration", assign.duration),
                        "source": "runway"
                    }
                    
            elif assign.source_type == VisualSource.STOCK:
                stock_asset = next(
                    (s for s in results["stock_assets"] if s.get("scene_id") == scene_id and s.get("url")),
                    None
                )
                if stock_asset:
                    fetched_assets[asset_key] = {
                        "url": stock_asset["url"],
                        "duration": stock_asset.get("duration", assign.duration),
                        "source": "stock"
                    }
                    
            elif assign.source_type == VisualSource.CONTENT:
                content_file = next(
                    (c for c in validated_content if c.get("id") == assign.content_file_id or c.get("name") == assign.content_file_id),
                    None
                )
                if content_file and content_file.get("url"):
                    content_url = content_file["url"]
                    content_id = content_file.get("id") or content_file.get("name")
                    
                    if content_file.get("file_type") == FileType.REFERENCE.value or content_file.get("_integration_blocked"):
                        print(f"[Execute] GUARDRAIL BLOCKED: Reference file attempted to enter fetched_assets for scene {scene_id}")
                        continue
                    
                    if content_url in reference_file_urls:
                        print(f"[Execute] GUARDRAIL BLOCKED: Content URL matches reference file registry for scene {scene_id}")
                        continue
                    if content_id in reference_file_ids:
                        print(f"[Execute] GUARDRAIL BLOCKED: Content ID matches reference file registry for scene {scene_id}")
                        continue
                    
                    fetched_assets[asset_key] = {
                        "url": content_url,
                        "duration": content_file.get("duration", assign.duration),
                        "source": "content"
                    }
        
        missing_assignments = []
        for assign in plan.source_assignments:
            asset_key = f"{assign.source_type.value}_{assign.scene_id}"
            if asset_key not in fetched_assets:
                missing_assignments.append(f"Scene {assign.scene_id} ({assign.source_type.value})")
        
        if missing_assignments:
            if require_complete_assets:
                results["errors"].append(f"INCOMPLETE ASSETS: Missing assets for scenes: {missing_assignments}")
                print(f"[Execute] ERROR: Missing assets for {len(missing_assignments)} assignments - HARD FAIL (require_complete_assets=True)")
                results["status"] = "failed"
                return results
            else:
                print(f"[Execute] WARNING: Missing assets for {len(missing_assignments)} assignments: {missing_assignments[:5]}")
                results["warnings"] = results.get("warnings", []) + [f"Missing assets for scenes: {missing_assignments}"]
        
        print(f"[Execute] Fetched assets map: {len(fetched_assets)} assets from {len(plan.source_assignments)} assignments")
    
    has_content = bool(fetched_assets)
    ...
    if has_content:
        if not SHOTSTACK_API_KEY:
            ...
            print("[Execute] Building Shotstack timeline from source assignments + skeleton timing...")
            
            if not plan.skeleton or not plan.source_assignments:
                results["errors"].append("REMIX MODE REQUIRES skeleton + source_assignments - cannot proceed without surgical plan")
                print("[Execute] ERROR: Remix mode requires skeleton and source_assignments for guardrail enforcement")
                results["status"] = "failed"
                return results
            
            clips = build_timeline_from_assignments(
            ...
            if clips:
                valid_asset_urls = {v["url"] for v in fetched_assets.values() if v.get("url")}
                for clip in clips:
                    if clip.asset_type == "video" and clip.asset_url not in valid_asset_urls:
                        results["errors"].append(f"GUARDRAIL VIOLATION: Clip asset_url not in fetched_assets: {clip.asset_url[:50]}...")
                        print(f"[Execute] ERROR: Asset URL not in fetched_assets map - blocking Shotstack submission")
                        results["status"] = "failed"
                        return results
                
                print(f"[Execute] Validation passed: {len(clips)} clips, all asset URLs verified in fetched_assets")
