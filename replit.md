...
- **Scene 1→2 Transition Preview** (`services/preview_service.py`): When a scene plan is built, automatically generates a transition preview showing Scene 1 flowing into Scene 2. Flow: DALL-E creates base frames for both scenes → Runway generates 5-second clips for each → FFmpeg stitches them with the planned transition (crossfade/cut). Runs asynchronously (threaded) so scene plan displays immediately. Frontend polls every 5 seconds with live status updates (Creating Scene 1 frame → Generating motion → Creating Scene 2 → Stitching transition). Graceful fallbacks: Scene 1 only if Scene 2 fails, DALL-E frame if Runway fails, error message if both fail.
