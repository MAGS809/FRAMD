...
@app.route('/analyze-edit', methods=['POST'])
def analyze_edit():
    """Analyze document edits and provide AI feedback with suggested actions."""
    from openai import OpenAI
    import os
    ...
    system_prompt = """You are Echo Engine, a document editor AI assistant for Framd.
    
Your job is to analyze the user's content and provide:
1. Brief, helpful feedback (1-2 sentences max)
2. Suggested next actions

ANALYZE THE CONTENT:
- Is this an idea/concept that needs a script?
- Is this already a script that needs refinement?
- Is this a transcript that could be clipped?

FEEDBACK RULES:
- Be concise and specific
- Focus on what would make the content stronger
- Suggest the logical next step

RESPOND IN JSON FORMAT:
{
    "feedback": "Your brief feedback here",
    "content_type": "idea|script|transcript",
    "ready_for_script": true/false,
    "actions": [
        {"type": "generate_script", "label": "Generate Script"},
        {"type": "refine", "label": "Refine Content"},
        {"type": "cast_voices", "label": "Assign Voices"},
        {"type": "find_visuals", "label": "Find Visuals"}
    ]
}

Only suggest relevant actions. If it's a raw idea, suggest generate_script.
If it's already a script, suggest cast_voices or find_visuals."""
    ...
    try:
        ...
        try:
            ...
            return jsonify({
                ...
                'content_type': result.get('content_type', 'idea'),
                'ready_for_script': result.get('ready_for_script', False)
            })
        except json.JSONDecodeError:
            return jsonify({
                'feedback': reply[:200] if len(reply) > 200 else reply,
                'actions': [{"type": "generate_script", "label": "Generate Script"}]
            })
            
    except Exception as e:
        logging.error(f"Analyze edit error: {e}")
        return jsonify({'feedback': None, 'error': str(e)})
